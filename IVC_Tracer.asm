...
;USART INIT
	...
	; Перейти на метку, если флаг установлен
	; Branch if Flag is Set
	; Первый параметр - флаг
	; Второй параметр - метка
	.MACRO BRFS
			sbrc	Flags,@0	; если флаг установлен,
			rjmp	@1			; то переходим
	.ENDM

	; Перейти на метку, если флаг очищен
	; Branch if Flag is Clear
	; Первый параметр - флаг
	; Второй параметр - метка
	.MACRO BRFC
			sbrs	Flags,@0	; если флаг очищен,
			rjmp	@1			; то переходим
	.ENDM
...
;------------------------------------------------
; период переполнения таймера T1                 |
...
BTN_PRESS_EVENT:
			...
			; 2. Минимальное значение ЦАП для АСВАХ
			; 3. Максимальное значение ЦАП для АСВАХ
			; 4. Шаг ЦАП для АСВАХ
...
; значение нулевого канала АЦП (ток солнечного модуля)
			;lds		XL,ADC_CH0+1
			;lds		XH,ADC_CH0+0
			;LCD_COORD 10,0		; курсор
			;rcall	DEC4_TO_LCD	; "сырое" значение АЦП
			; переводим в миливольты
			;lds		r16,ADC_CH0+1	; младший байт АЦП
			;lds		r17,ADC_CH0+0	; старший байт АЦП
			;ldi		r18,low(1221)
			;ldi		r19,high(1221)
...
;------------------------------------------------------------------------------
; Вывод 4-значного десятичного числа в UART напрямую
; Вход: число в регистре X
;------------------------------------------------------------------------------
DEC4_TO_UART_SIMPLE:
			rcall	Bin2ToBCD4
...
;------------------------------------------------------------------------------
; Для вывода напряжения и "тока" солнечного модуля на дисплей
;------------------------------------------------------------------------------
BCD_TO_LCD:
...
;------------------------------------------------------------------------------
; Для вывода напряжения и "тока" солнечного модуля в UART
;------------------------------------------------------------------------------
BCD_TO_UART:
			mov		r16,r28		; берем первую цифру
			andi	r16,0x0F	; обнуляем старшую тетраду
			subi	r16,-0x30	; переводим в ASCII код
			rcall	uart_snt	; выводим
